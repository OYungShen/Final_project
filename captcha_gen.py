...
class captchatext:
    def __init__(self, priority, offset, captchalen, engletter, ENGNOLIMIT):
        ...
        if ENGNOLIMIT:
            engletter = True if randint(1, 28) <= 21 else False
        if engletter:
            self.letter = ENGSTR[randint(0, len(ENGSTR) - 1)]
        else:
            self.letter = str(number[randint(0, len(number) - 1)])
...
def generate(GENNUM, SAVEPATH, ENGP=25, FIVEP=0, ENGNOLIMIT=False, filename="train"):
    ...
    for index in range(1, GENNUM + 1, 1):
        captchastr = ""
        captchalen = 5 #f randint(1, 100) <= FIVEP else 6
        engat = randint(0, captchalen - 1) if randint(1, 100) <= ENGP else -1
        bgcolor = [randint(180, 250) for _ in range(3)]
        captcha = Image.new('RGBA', (160, 60), (bgcolor[0], bgcolor[1], bgcolor[2], 255))
        ...
        captcha.convert("RGB").save(SAVEPATH + str(captchastr)+"_"+now +".jpg", "JPEG")
...
if __name__ == "__main__":
    generate(70000, "./data/5_train_set/",  ENGP=100, FIVEP=50, ENGNOLIMIT=True, filename="train")
    ...
    #generate(1, "./data/5_vali_set/",  ENGP=100, FIVEP=100, ENGNOLIMIT=True, filename="vali")
